Shader "Custom/GeoTrimShader"
{
    Properties
    {
        ...
        [PerRendererData] trimAngle ("trimAngle", float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 calculateGerstnerWave(float2 pos, float4 waveData){
                ...
                //Calculated position transform depends on rotation angle
                if(floor(trimAngle) == 0){
                    //0 degrees
                    return float3(d.x * (a * cos(f)), 
                                        a * sin(f), 
                                d.y * (a * cos(f)));
                } else if (floor(trimAngle) == 4){
                    //90 degrees?
                    return float3(-d.y * (a * cos(f)), 
                                        a * sin(f), 
                                  d.x * (a * cos(f)));
                } else if (floor(trimAngle) == 1){
                    //270 degrees
                    return float3(d.y * (a * cos(f)), 
                                        a * sin(f), 
                                  -d.x * (a * cos(f)));
            ...
            { 
                ...
                IN.uv = IN.uv - pivot;
                float2x2 rotation = float2x2(cos(trimAngle), -sin(trimAngle), sin(trimAngle), cos(trimAngle));
                IN.uv = mul(rotation, IN.uv);
                IN.uv += pivot;
                ...
                //Use modified position in normal calculationa
